$(document).ready(function () {
    ...
    $(".city-results").on("click", "li", function () {
        var clickEl = $(this)[0].textContent;
        $("#city-name-title").text(clickEl);
        $(".weather-card-body").text(clickEl);
        console.log(clickEl);
        //fiveDayForecast();
        getCoordinates(clickEl);
    });
    ...
    function addToStorage() {
        //push to cityStorage
        localStorage.setItem('city-current', JSON.stringify(cityStorage));
    };
    ...
    function historySearch(event) {
        ...
        handleSearch(city)
        //have a city history function that gets the global array from local storage
        //and makes a <ul> with <li> for each city - make sure to add a class (searchedCities) and an id. I would suggest having the city name as the id
        //Add a click event on searchedCities that grabs the id (city name)
        //that would call your getCoordinates function to redo the search
    ...
    //cityResults.on("click", handleSearch);
